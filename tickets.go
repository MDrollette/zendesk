...
func (api *TicketApi) putTicket(path string, payload interface{}) (Ticket, error) {
	response := struct {
		Ticket Ticket `json:"ticket"`
	}{}
...
func (api *TicketApi) UpdateTicket(ticket Ticket) (Ticket, error) {
	path := fmt.Sprintf("/api/v2/tickets/%d.json", ticket.Id)
	return api.putTicket(path, map[string]Ticket{"ticket": ticket})
}
